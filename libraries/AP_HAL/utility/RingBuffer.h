...
/*
  Non-thread safe ring buffer class for objects of fixed size.
 */
template <class T>
class ObjectBuffer {
public:
...
/*
  ...
  Simple wrapper around ObjectBuffer with scoped semaphore.
 */
template <class T>
class ObjectBuffer_TS : ObjectBuffer<T> {
public:
    ObjectBuffer_TS(uint32_t _size = 0) {
    }

    ObjectBuffer_TS(ByteBuffer *_buffer) :
    ObjectBuffer<T>::buffer(_buffer),
    ObjectBuffer<T>::external_buf(true)
    {}

    // return size of ringbuffer
    uint32_t get_size(void) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::get_size();
    }

    // set size of ringbuffer, caller responsible for locking
    bool set_size(uint32_t size) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::set_size(size);
    }

    // read len objects without advancing the read pointer
    uint32_t peek(T *data, uint32_t len) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::peek(data, len);
    }
    ...
    uint32_t available(void) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::available();
    }

    // return number of objects that could be written to the back of the queue
    uint32_t space(void) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::space();
    }

    // true is available() == 0
    bool is_empty(void) WARN_IF_UNUSED {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::is_empty();
    }

    // push one object onto the back of the queue
    bool push(const T &object) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::push(object);
    }

    // push N objects onto the back of the queue
    bool push(const T *object, uint32_t n) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::push(object, n);
    }
    ...
    bool pop(T &object) WARN_IF_UNUSED {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::pop(object);
    }
    ...
    bool push_force(const T &object) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::push_force(object);
    }
    ...
    bool push_force(const T *object, uint32_t n) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::push_force(object, n);
    }
    ...
    bool peek(T &object) WARN_IF_UNUSED {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::peek(object);
    }
    ...
    const T *readptr(uint32_t &n) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::readptr(n);
    }

    // advance the read pointer (discarding objects)
    bool advance(uint32_t n) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::advance(n);
    }
    ...
    bool update(const T &object) {
        WITH_SEMAPHORE(sem);
        return ObjectBuffer<T>::update(object);
    }
